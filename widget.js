...
requirejs.config({
    ...
    */
    paths: {
        Three: 'https://cdnjs.cloudflare.com/ajax/libs/three.js/89/three',
        ...
        onDropped: function (data, info) {
            ...
            if (droppedFile !== undefined) {
                ...
                droppedFile = Board.supportedFiles.find(function(f){
                    ...
                    return info.name.match(extRegEx);
                });
                if (droppedFile !== undefined) {
                    chilipeppr.publish('/com-chilipeppr-elem-flashmsg/flashmsg', "Error Loading Board File", "Looks like you dragged in supported board file, but it seems to be in binary. You can open this file in Eagle and then re-save it to a new file to create a text version of your Eagle BRD file.", 15 * 1000);
                    return false;
                }
            }
        },
        ...
        */
            ...
            paths.forEach(function(signalPart){
                ...
                signalPart.holes.forEach(function(holePath){
                    ...
                    holePath.forEach(function(vertex){
                        if(firstVetex)
                            {hole.moveTo(vertex.X, vertex.Y); firstVetex = false;}
                        else
        ...
        createWirePath: function(x1, y1, x2, y2, width, curve){
            ...
            if(curve == 0){
                ...
                var p1x = x1 + dx1, p1y = y1 - dy1, p2x = x2 + dx2, p2y = y2 - dy2,
                    p3x = x2 - dx2, p3y = y2 + dy2, p4x = x1 - dx1, p4y = y1 + dy1;
...
PATHS.createCircularPath = function (x, y, radius, seg){
    ...
    for(var a = 2 * Math.PI; a > 0; a-=step){
        path.push({X: x + radius * Math.cos(a + Math.PI /8),
...
PATHS.createOvalPath = function (x, y, sizeX, sizeY, seg){
    ...
    if(sizeX > sizeY){
        dx = sizeX / 4;
        sa = 3*Math.PI / 2; ea = - Math.PI / 2; ma = Math.PI / 2;
    }
    else {
        dy = (sizeX == sizeY)? 0 : sizeY / 4;
        sa = 2 * Math.PI; ea = 0; ma = sa / 2;
    }
...
function Board3d(){
    ...
    Board3d.prototype.center = function(){
        ...
        var center = new THREE.Vector3(
                - (groupBox.min.x + (groupBox.max.x - groupBox.min.x) / 2),
                - (groupBox.min.y + (groupBox.max.y - groupBox.min.y) / 2),
                - (groupBox.min.z + (groupBox.max.z - groupBox.min.z) / 2));
        this.group.traverse(function (child) {
            if (child instanceof THREE.Mesh) {
...
Board.prototype.parseEagle = function () {
        ...
        Array.from(xmlVias).forEach(function(xmlVia){
            ...
            this.layers.forEach(function(layer){
                if(layer.signalLayer){
                    ...
                    if(layer.key == 1 || layer.key == 16)
                        {via.diameter = via.diameterOuter; }
                    else
                        {via.diameter = via.diameterInner; }
    ...
    for(var elemKey in this.elementsByName){
        ...
        elmPkg.pads.forEach(function (xmlPad){
            for(var l = 1; l <= 16; l++){
                ...
                if(layer){
                    ...
                    if(l == 1)
                        ...
                        {pad.sizeX = pad.sizeY = pad.diameterBottom; }
                    else
                        {pad.sizeX = pad.sizeY = pad.diameterInner; }
            ...
            this.holes.push({
                x: pad.x, y: pad.y, drill: pad.drill, type: "pad",
                packageX: elem.x, 
                packageY: elem.y, 
        ...
        elmPkg.holes.forEach(function (hole){
            this.holes.push({
                x: hole.x, y: hole.y, drill: hole.drill, type: "package",
                packageX: elem.x, 
                packageY: elem.y, 
                packageMirror: elem.mirror, 
                packageRotation: elem.rotation});
...
Board.prototype.parseKiCad = function () {
    ...
    Array.from(xmlPads).forEach(function(xmlPad){
        ...
        if(pad.type == "thru_hole")
            this.holes.push({
                x: pad.x, y: pad.y, type: "package",
                drill: pad.drill, drillX: pad.drillX, drillY: pad.drillY, drillShape: pad.drillShape, revolve: pad.revolve,
        ...
        while(cbi<text.length - 1 && obi != -1 && cbi != -1){
            ...
            if(attributes.hasChildren){
                ...
                if(parent.nodeName == "layers"){
                    ...
                    var w,h;
                    if(vals[1]=="User") {w = vals[2]; h = vals[3];}
                    else if(vals[1]=="A4") {w = 210; h = 297;}
                    else if(vals[1]=="A3") {w = 297; h = 420;}
                    else if(vals[1]=="A2") {w = 420; h = 594;}
                    else if(vals[1]=="A1") {w = 584; h = 841;}
                    else if(vals[1]=="A0") {w = 841; h = 1189;}
                    else if(vals[1]=="A" || vals[1]=="USLetter") {w = 8.5*25.4; h = 11*25.2;}
                    else if(vals[1]=="B" || vals[1]=="USLedger") {w = 11*25.4; h = 17*25.2;}
                    else if(vals[1]=="C") {w = 17*25.4; h = 22*25.2;}
                    else if(vals[1]=="D") {w = 22*25.4; h = 34*25.2;}
                    else if(vals[1]=="E") {w = 34*25.4; h = 44*25.2;}
                    else if(vals[1]=="USLegal") {w = 8.5*25.4; h = 14*25.2;}
